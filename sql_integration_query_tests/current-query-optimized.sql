-- Optimized candidate for current-query.sql (same result contract).
-- Direct-test configuration copied from v2-better-query-01.sql.
-- Conservative optimization: index-friendly statistics timestamp ranges.
...
/* Compute the history boundary once. The production predicate is
   local_date(row) > local_date(now - 90 days), hence local midnight -89 days. */
clock AS (
    SELECT
        CAST(strftime('%s', date('now', offset, '-89 days')) AS INTEGER) - offset_seconds as history_start_ts
    FROM vars
),
...
weather_history_raw AS (
    ...
    WHERE s.metadata_id = (SELECT cloud_id_states FROM ids)
      AND ((SELECT sensor_clouds FROM vars) LIKE 'weather.%' OR NOT EXISTS (SELECT 1 FROM statistics WHERE metadata_id = (SELECT cloud_id_statistics FROM ids) AND start_ts >= (CAST(s.last_updated_ts AS INT) / 3600) * 3600
    ...
    WHERE s.metadata_id = (SELECT w_entity_id FROM ids)
      AND ((SELECT sensor_clouds FROM vars) LIKE 'weather.%' OR NOT EXISTS (SELECT 1 FROM statistics WHERE metadata_id = (SELECT cloud_id_statistics FROM ids) AND start_ts >= CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars)
      ...
      AND NOT EXISTS (SELECT 1 FROM statistics WHERE metadata_id = (SELECT cloud_id_statistics FROM ids)  AND start_ts >= CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars)
          AND start_ts < CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars) + 86400)
    ...
    WHERE s.metadata_id = (SELECT uv_id_states FROM ids)
      AND NOT EXISTS (SELECT 1 FROM statistics WHERE metadata_id = (SELECT uv_id_statistics FROM ids)  AND start_ts >= CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars)
          AND start_ts < CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars) + 86400)
      AND s.last_updated_ts > strftime('%s', 'now', '-10 days')
      AND s.state NOT IN ('unknown', 'unavailable', '')
    ...
    WHERE s.metadata_id = (SELECT temp_id_states FROM ids)
      AND NOT EXISTS (SELECT 1 FROM statistics WHERE metadata_id = (SELECT temp_id_statistics FROM ids)  AND start_ts >= CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars)
          AND start_ts < CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars) + 86400)
      AND s.last_updated_ts > strftime('%s', 'now', '-10 days')
      AND s.state NOT IN ('unknown', 'unavailable', '')
    ...
    WHERE s.metadata_id = (SELECT precip_id_states FROM ids)
      AND NOT EXISTS (SELECT 1 FROM statistics WHERE metadata_id = (SELECT precip_id_statistics FROM ids) AND start_ts >= CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars)
          AND start_ts < CAST(strftime('%s', date(s.last_updated_ts, 'unixepoch', (SELECT offset FROM vars))) AS INTEGER) - (SELECT offset_seconds FROM vars) + 86400)
      AND s.last_updated_ts > strftime('%s', 'now', '-10 days')
      AND s.state NOT IN ('unknown', 'unavailable', '')
),
